...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if self.can_contain(place.ant) or place.ant.can_contain(self):
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        p = self.place
        distance = 0
        while distance < self.min_range and p.entrance:
            p, distance = p.entrance, distance + 1
        while p.entrance != beehive and (not p.bees) and distance < self.max_range and p.entrance:
            if not p.bees:
                p, distance = p.entrance, distance + 1
...
class LongThrower(ThrowerAnt):
    ...
    # OVERRIDE CLASS ATTRIBUTES HERE
    # BEGIN Problem 4
    implemented = True   # Change to True to view in the GU
    min_range = 5
    max_range = float('inf')
    # END Problem 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        place_bees_copy = place.bees[:]
        if place.bees:
            for bee in place_bees_copy:
                Insect.reduce_armor(bee, amount)
        if self.armor == 0 and place.bees:
            for bee in place_bees_copy:
...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt is an ant that does nothing each turn
    """
    implemented = True
    name = 'Wall'
    food_cost = 4
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem Optional 4
        "*** YOUR CODE HERE ***"
        if target and not target.already_scared:
        ...
        while place != beehive:
            ...
            if place.ant is not None and place.ant is not self :
                ...
                if isinstance(place.ant, ContainerAnt):
                    insects_front[place.ant.contained_ant] = distance
            """deal with the bees"""
            if place.bees:
